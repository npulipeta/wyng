...
function getReqQuantity (handlerInfo, values) {
    return new Promise((resolve, reject) => {
        ...
        dbHandler.mysqlQueryPromise(handlerInfo, "getReqQuantity" , query, values).then((result) => {
            //logging.logDatabaseQuery(handlerInfo, 'getReqQuantity', null, result, query.sql);
            resolve(result);
        }, (error) => {
            reject(error);
        });
